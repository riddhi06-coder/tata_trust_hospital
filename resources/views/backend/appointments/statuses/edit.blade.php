                        ...
                        <div class="card-body">
                            <form method="POST" action="{{ route('manage-appointment-statuses.update', $status->id) }}">
                                ...
                                <div class="form-check form-switch mb-3">
                                    <input class="form-check-input" type="checkbox" name="requires_appointment_date" value="1" id="requiresDate" {{ old('requires_appointment_date', $status->requires_appointment_date) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="requiresDate">Require a new appointment date (e.g. Rescheduled)</label>
                                </div>
                                @php $curTrigger = old('sms_trigger', $status->sms_trigger); @endphp
                                <div class="mb-4 col-md-5">
                                    <label class="form-label">Send SMS when this status is applied</label>
                                    <select name="sms_trigger" class="form-select">
                                        <option value="" {{ !$curTrigger ? 'selected' : '' }}>No SMS</option>
                                        <option value="cancellation" {{ $curTrigger === 'cancellation' ? 'selected' : '' }}>Cancellation SMS</option>
                                        <option value="reschedule" {{ $curTrigger === 'reschedule' ? 'selected' : '' }}>Reschedule SMS</option>
                                    </select>
