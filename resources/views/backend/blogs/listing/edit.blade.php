...
<body>
    ...
    @php
        $bannerUrl    = $showBanner && $settings && $settings->banner_image
            ? asset('home/blog/banner/'.$settings->banner_image)
            : '';
        $thumbnailUrl = $listing->thumbnail ? asset('home/blog/thumbnails/'.$listing->thumbnail) : '';
    @endphp
    ...
    <div class="page-body">
        <div class="container-fluid">
            <div class="page-title">
                <div class="row">
                    <div class="col-6"><h4>Edit Blog</h4></div>
                    <div class="col-6">
                        <ol class="breadcrumb">
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Update Blog</h4>
                            @if($showBanner)
                                <p class="f-m-light mt-1">This is the first blog — banner details for the whole blog page are edited here.</p>
                            @else
                                <p class="f-m-light mt-1">Update the blog details below.</p>
                            @endif
                        </div>
                        <div class="card-body">
                            ...
                            <form class="row g-4 custom-input" action="{{ route('manage-blogs-listing.update', $listing->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                        ...
                                        <div class="row g-3">
                                            <div class="col-md-6">
                                                ...
                                                <select class="form-select" id="blog_category_id" name="blog_category_id">
                                                    ...
                                                    @foreach($categories as $category)
                                                        <option value="{{ $category->id }}" {{ (string) old('blog_category_id', $listing->blog_category_id) === (string) $category->id ? 'selected' : '' }}>
                                                            {{ $category->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <input class="form-control" id="blog_date" type="date" name="blog_date"
                                                    value="{{ old('blog_date', optional($listing->blog_date)->format('Y-m-d')) }}">
                                            </div>

                                            <div class="col-md-12">
                                                <label class="form-label" for="title">Blog Title <span class="txt-danger">*</span></label>
                                                <input class="form-control" id="title" type="text" name="title"
                                                    value="{{ old('title', $listing->title) }}" placeholder="Enter Blog Title">
                                                <small class="text-muted">Slug regenerates only if the title changes. Current slug: <code>{{ $listing->slug }}</code></small>
                                            </div>
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <div class="mt-2">
                                                    <img id="thumbnail-preview"
                                                        src="{{ $thumbnailUrl }}"
                                                        data-existing="{{ $thumbnailUrl }}"
                                                        alt=""
                                                        style="max-height:140px; max-width:220px; object-fit:cover; border-radius:6px; border:1px solid #e5e7eb; {{ $thumbnailUrl ? '' : 'display:none;' }}">
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <textarea class="form-control ckeditor-init" id="short_description" name="short_description" rows="6"
                                                    placeholder="Enter Short Description">{{ old('short_description', $listing->short_description) }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <table class="table table-bordered align-middle" id="tag-table">
                                                ...
                                                <tbody id="tag-tbody">
                                                    @forelse($tags as $i => $t)
                                                        <tr data-row="{{ $i }}">
                                                            <td>
                                                                <input type="hidden" name="tags[{{ $i }}][id]" value="{{ $t->id }}">
                                                                <input type="text" name="tags[{{ $i }}][tag]" class="form-control"
                                                                    value="{{ old('tags.'.$i.'.tag', $t->tag) }}"
                                                                    placeholder="e.g. Pet Care, Feeding Tips, Puppies">
                                                            </td>
                                                    ...
                                                    @empty
                                                        <tr data-row="0">
                                                            ...
                                                            <td class="text-center">
                                                                <button type="button" class="btn btn-danger btn-sm remove-tag-row" disabled>Remove</button>
                                                            </td>
                                                        </tr>
                                                    @endforelse
                                                </tbody>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            /* ---------- File input previews with keep-existing ---------- */
            ...
            var tagIdx = tagTbody.querySelectorAll('tr').length;
