            ...
            <tr>
              <td style="padding:30px 36px 10px;color:#333333;">
                ...
                <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;font-size:14px;">
                  <tr>
                    <td style="padding:10px 0;border-bottom:1px solid #eee;color:#888;width:42%;">Applying For</td>
                    <td style="padding:10px 0;border-bottom:1px solid #eee;color:#222;font-weight:bold;">{{ $application->applying_for }}</td>
                  </tr>
                  <tr>
                    <td style="padding:10px 0;border-bottom:1px solid #eee;color:#888;">Name</td>
                    <td style="padding:10px 0;border-bottom:1px solid #eee;color:#222;font-weight:bold;">{{ $application->full_name }}</td>
                  </tr>
                  <tr>
                    <td style="padding:10px 0;border-bottom:1px solid #eee;color:#888;">Email</td>
                    <td style="padding:10px 0;border-bottom:1px solid #eee;color:#222;font-weight:bold;">{{ $application->email }}</td>
                  </tr>
                  <tr>
                    <td style="padding:10px 0;border-bottom:1px solid #eee;color:#888;">Phone</td>
                    <td style="padding:10px 0;border-bottom:1px solid #eee;color:#222;font-weight:bold;">{{ $application->phone }}</td>
                  </tr>
                  @if(!empty($application->location))
                    <tr>
                      <td style="padding:10px 0;border-bottom:1px solid #eee;color:#888;">Current Location</td>
                      <td style="padding:10px 0;border-bottom:1px solid #eee;color:#222;font-weight:bold;">{{ $application->location }}</td>
                    </tr>
                  @endif
                  @if(!empty($application->joining_time))
                    <tr>
                      <td style="padding:10px 0;border-bottom:1px solid #eee;color:#888;">Can Join In</td>
                      <td style="padding:10px 0;border-bottom:1px solid #eee;color:#222;font-weight:bold;">{{ $application->joining_time }}</td>
                    </tr>
                  @endif
                  @if(!empty($application->message))
                    <tr>
                      <td style="padding:10px 0;border-bottom:1px solid #eee;color:#888;vertical-align:top;">Message</td>
                      <td style="padding:10px 0;border-bottom:1px solid #eee;color:#222;">{!! nl2br(e($application->message)) !!}</td>
                    </tr>
                  @endif
                </table>
                @if(!empty($note))
